import torch, itertools, time, csv  # <-- MODIFIED
...
def train():
	...
	csv_path = os.path.join(opt.checkpoints_dir, opt.name, 'loss_log.csv')  # <-- NEW
	os.makedirs(os.path.dirname(csv_path), exist_ok=True)
	with open(csv_path, mode='w', newline='') as file:  # <-- NEW
	...
	for epoch in tqdm(range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1), desc="Epochs"):
		start_time = time.time()

		# Cumul des pertes pour la moyenne
		total_loss_G, total_loss_G_A, total_loss_G_B = 0, 0, 0
		total_loss_cycle_A, total_loss_cycle_B = 0, 0
		total_loss_idt_A, total_loss_idt_B = 0, 0
		total_loss_D_A, total_loss_D_B = 0, 0
		n_batches = 0
		...
		with open(csv_path, mode='a', newline='') as file:  # <-- NEW
			writer = csv.writer(file)
			writer.writerow([
				epoch,
